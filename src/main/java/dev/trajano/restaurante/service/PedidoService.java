...
import dev.trajano.restaurante.models.entity.*;
...
public class PedidoService {
    ...
    public PedidoResponse criarPedido(PedidoRequest request){
        ...
        Garcom garcom = garcomRepository.findById(request.garcomId())
                .orElseThrow(()-> new NotFoundException("Garcom nao encontrado!"));
        Mesa mesa = mesaRepository.findById(request.mesaId())
                .orElseThrow(()-> new NotFoundException("Mesa nao encontrado!"));
    ...
    public PedidoResponse atualizarPedidoStatus(Long id,StatusPedido status) {
        ...
        if (!pedido.getStatus().podeTracionarPara(status)) {
            throw new MethodArgumentNotValidException("Status nao pode ser alterado!");
        }
    ...
    public PedidoResponse removerItem(Long pedidoId, Long itemId){
        ...
        if (pedido.getItens().size()==1)
            throw new MethodArgumentNotValidException("Produto nao encontrado!");
